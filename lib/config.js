...
type Plugin = string | PluginObject
...
export function createPatternObject (pattern: ?PatternLike) {
  if (pattern == null) {
  ...
  if (pattern.pattern && typeof pattern.pattern === 'string') {
    if (helper.isUrlAbsolute(pattern.pattern)) {
      return new UrlPattern(pattern.pattern)
    }
...
var normalizeUrl = function (url: string): string {
  if (url.charAt(0) !== '/') {
    url = '/' + url
  }
...
var normalizeUrlRoot = function (urlRoot: string): string {
...
var normalizeProxyPath = function (proxyPath: string): string {
...
var normalizeConfig = function (config: {[id:string]: any}, configFilePath: string) {
  var basePathResolve = function (relativePath: string) {
  ...
  var createPatternMapper = (resolve: (string) => string) => {
    return (objectPattern: Pattern): Pattern => {
      if (objectPattern.pattern != null) {
  ...
  if (Array.isArray(config.files)) {
    ...
    log.warn('Invalid files property')
    config.files = []
  }
  ...
  if (typeof config.reporters === 'string') {
  ...
  if (config.formatError && typeof config.formatError == 'function') {
  ...
  Object.keys(defaultClient).forEach((key) => {
    if (config.client[key] == null) {
      config.client[key] = defaultClient[key]
    }
  })

  // normalize preprocessors
  var preprocessors = config.preprocessors || {}
  var normalizedPreprocessors = config.preprocessors = {}
...
export class Config {
  ...
  port: number
  hostname: string
  httpsServerConfig: {[id:string]: any}
  basePath: string
  files: Pattern[]
  ...
  proxies: {[id:string]: any}
  proxyValidateSSL: boolean
  preprocessors: {[id:string]: any}
  urlRoot: string
  ...
  set (newConfig: {[id:string]: any}) {
    _.mergeWith(this, normalizeConfig(newConfig, this.configFilePath), (obj, src) => {
  ...
  constructor (newConfig: ?{[id:string]: any}, configFilePath: string) {
...
export function parseConfig (configFilePath: string, cliOptions: {[id:string]: string}): Config {
  ...
  var config = new Config(cliOptions, configFilePath)
