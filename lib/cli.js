...
type ProcessOptions = {[id:string]: any}

const processArgs = function (argv: {[id:string]: any}, options: ProcessOptions, fs: Fs, path: Path) {
  if (argv.help) {
  ...
  if (helper.isString(options.formatError)) {
    ...
    if (required && required.formatError) {
      options.formatError = required.formatError
    } else {
      options.formatError = required
    }
  ...
  options.configFile = typeof configFile === 'string' ? path.resolve(configFile) : null

  return options
}

var parseClientArgs = function (argv: string[]): string[] {
...
var argsBeforeDoubleDash = function (argv: string[]): string[] {
  var idx = argv.indexOf('--')
